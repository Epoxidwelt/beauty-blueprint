...
**Homecare-Produktauswahl:** Auf dem Empfehlungen-Screen zeigen zwei getrennte Karten (Morgens/Abends) das vom System zur Priorität passende Produkt vorausgewählt. Über „Anderes Produkt wählen" öffnet sich eine Suche (`ALL_PRODUCTS` in `prototype.html`, alle 364 echten KLAPP-Homecare-Produkte aus dem Studiolution-Export) — Tippen filtert live, ohne dass das Eingabefeld den Fokus verliert. Eine Auswahl ersetzt den Systemvorschlag durchgängig (Empfehlungen, Beauty Blueprint, E-Mail-Zusammenfassung); „Systemvorschlag verwenden" macht das rückgängig. Jeder Slot lässt sich zusätzlich unabhängig entfernen.
